...
export const BookingDialog = ({ businessId, open, onOpenChange, onSuccess }: BookingDialogProps) => {
  ...
  const [overlapInfo, setOverlapInfo] = useState<{ conflictingBooking: any; startTime: Date; endTime: Date } | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent, forceCreate = false) => {
    ...
    try {
      ...
      // Check for double-booking if staff is selected (unless user confirmed to proceed)
      if (formData.staff_id && !forceCreate) {
        ...
        if (hasOverlap) {
          ...
          setOverlapInfo({ conflictingBooking, startTime, endTime });
      ...
      await createBooking(startTime, endTime);
    } catch (error: any) {
  ...
  const handleConfirmOverlap = async () => {
    ...
    setLoading(true);
    try {
      await createBooking(overlapInfo.startTime, overlapInfo.endTime);
    } catch (error: any) {
      console.error("Booking creation error:", error);
      toast({
        title: "Error",
  ...
  const createBooking = async (startTime: Date, endTime: Date) => {
  ...
  return (
    ...
    <AlertDialog open={showOverlapConfirm} onOpenChange={setShowOverlapConfirm}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Double Booking Detected</AlertDialogTitle>
          <AlertDialogDescription>
            {overlapInfo?.conflictingBooking && (
              <>
        ...
        <AlertDialogFooter>
          <AlertDialogCancel onClick={() => setOverlapInfo(null)}>Cancel</AlertDialogCancel>
          <AlertDialogAction onClick={handleConfirmOverlap} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
            Create Anyway
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
